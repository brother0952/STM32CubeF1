/**
  ...
  可以使用。
  出错的时候要插拔usb口，在中断的地方进入断点通常会出错，需要插拔
  
  usb口接收的数据会转发给串口1，pa9，pa10
  两边波特率需一致，可以短接pa9和10做测试

  TODO 不经过串口1，usb接收数据，处理数据后从usb口返回

  stm3210c_eval 是stm32f107  和103差异较大，先不要使用 它的例程，stm3210e_eval,STM32F103RB-Nucleo 和stm32vl-discovery 都不错
