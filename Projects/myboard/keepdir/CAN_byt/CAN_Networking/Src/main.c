...
int main(void)
{
  ...
  while(1){
		...
		if(BSP_PB_GetState(BUTTON_KEY) == KEY_PRESSED){//KEY_NOT_PRESSED
			key=1;
		}else{
			key=0;
		}
		...
		// if not pending
		if(0==HAL_CAN_IsTxMessagePending(&CanHandle,TxMailbox)) // no pending
		{
...
static void CAN_Config(void)
{
  ...
  CanHandle.Init.Prescaler = 8;//500k
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *CanHandle)
{
  ...
  /* Display LEDx */
  if ((RxHeader.StdId == 0x322) && (RxHeader.IDE == CAN_ID_STD) && (RxHeader.DLC == 8))
  {
